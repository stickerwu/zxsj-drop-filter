...
- 支持玩家手动滚动、画面停稳后自动采集，`Ctrl+Shift+F8` 可用于手动补采
- 支持多个游戏窗口选择、低置信度修正和漏项手动补充

### 库存扫描使用方法

1. 启动《诛仙世界》，请打开个人游戏角色的“天工机巧盘”页面并进入机巧石库存，游戏窗口不能最小化。
2. 在工具箱的“天工机巧盘”模式点击“库存扫描”，选择游戏窗口并开启扫描。
3. 在游戏中切换“机巧石 / 匠心石”页签并手动滚动列表，每次保留至少一行重叠；画面停稳约 0.35 秒后自动采集。
4. 在独立核对窗口修正低置信度字段、未知形状、误识别项和漏项。
5. 两个页签条目数与游戏显示总数一致后，点击“应用库存”同步到求解器。

连续扫描默认开启。同一静止画面只自动识别一次，失败后请调整滚动位置或使用
`Ctrl+Shift+F8` / “手动补采当前页”，避免 OCR 对错误画面循环重试。

### 安全与隐私边界

- 仅使用 Windows 窗口捕获和本地 OCR，不注入 DLL、不读取游戏内存、不分析封包。
- 自动采集只检测画面变化；不自动激活游戏、不发送鼠标键盘输入、不自动点击或滚动。
